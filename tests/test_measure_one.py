...
def test_run_measures_delta_over_launch_baseline(monkeypatch):
    ...
    monkeypatch.setattr(measure_one, "_spawn_worker",
                        lambda hf, ctx, abort_gb, kv_bits=None, prefer_flash=False, weight_quant="none", chunk=None: {"status": "ok", "used_gb": 4.0,
                                                   "baseline_gb": 2.0})
...
def test_run_passes_safe_budget_as_abort_limit(monkeypatch):
    ...
    def spy(hf, ctx, abort_gb, kv_bits=None, prefer_flash=False, weight_quant="none", chunk=None):
...
def test_run_median_over_repeats(monkeypatch):
    ...
    monkeypatch.setattr(measure_one, "_spawn_worker",
                        lambda hf, ctx, abort_gb, kv_bits=None, prefer_flash=False, weight_quant="none", chunk=None: {"status": "ok",
...
def test_run_refused_when_worker_not_ok(monkeypatch):
    _patch(monkeypatch)
    monkeypatch.setattr(measure_one, "_spawn_worker",
                        lambda hf, ctx, abort_gb, kv_bits=None, prefer_flash=False, weight_quant="none", chunk=None: {"status": "error", "note": "OOM"})
    r = measure_one.run("org/m", 2000, margin_gb=1.0, overhead_gb=0.6)
...
def test_run_threads_chunk_to_worker(monkeypatch):
    _patch(monkeypatch)
    seen = {}
    ...
    monkeypatch.setattr(measure_one, "_spawn_worker", spy)
    measure_one.run("org/m", 2000, margin_gb=1.0, overhead_gb=0.6, repeats=1, chunk=256)
...
def test_main_measure_prints_result(monkeypatch, capsys):
    _patch(monkeypatch)
    monkeypatch.setattr(measure_one, "_spawn_worker",
                        lambda hf, ctx, abort_gb, kv_bits=None, prefer_flash=False, weight_quant="none", chunk=None: {"status": "ok", "used_gb": 4.0, "baseline_gb": 2.0})
